...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # values for the thing
        tmp = []
        tmp_test = []
        tmp2 = []
        goalPossition = None # make sure this gets overwritten
        goalProbability = -1 # make sure this gets overwritten

        # get the beliefs max list
        for pos in livingGhostPositionDistributions:
            #tmp_test.append(pos)
            tmp.append(pos.argMax())

        # get best stuff
        for i, pos in enumerate(tmp):
            tmp_value = livingGhostPositionDistributions[i][pos]
            if tmp_value > goalProbability:
                goalPossition = pos
                goalProbability = tmp_value

        # do action stuff
        for action in legal: # get actions from given
            next = Actions.getSuccessor(pacmanPosition,action)
            tmp2.append((self.distancer.getDistance(next,goalPossition),action))

        # return best action
        return_val = min(tmp2)[1]
        return return_val